 ...
 * Update:
 * - Added Context Menu for Row/Column operations.
 * - Implemented insert/delete logic for specific indices.
 * - Refactored layout to CSS Grid to fix button overlap issues.
 * - Changed button visibility logic: buttons appear only when hovered directly.
 * - Fix: Prevent text immediately following the table from being merged into the table widget.
 */
...
const TableComponent = ({ initialData, initialWidths, onUpdate, onRender }) => {
    ...
    const insertRow = (index) => {
        ...
        // For splice, we need array index. 
        // rows array index 0 corresponds to rowIndex 0.
    ...
    const handleMenuAction = (action, rowIndex, colIndex) => {
        ...
        switch (action) {
            case 'insertRowAbove':
                // ヘッダー(-1)の上には挿入できないので、ヘッダー下(=0)に挿入するか、あるいは無効化
                // Notionではヘッダー上挿入はないが、ここでは「行番号」の位置に挿入する
                // rowIndex -1 の場合、0番目に挿入＝ヘッダーと1行目の間
    ...
    syntaxTree(state).iterate({
        enter: (node) => {
            if (node.name === "Table") {
                ...
                // ★修正: テーブル末尾にパイプを含まない行が含まれている場合、それを範囲から除外する
                // これにより、テーブル直下に入力されたテキストがテーブルの一部として取り込まれるのを防ぐ
                let adjustedTo = tableTo;
                while (adjustedTo > tableFrom) {
                    const line = state.doc.lineAt(adjustedTo - 1);
                    if (line.from < tableFrom) break; // 安全策

                    const text = line.text;
                    // パイプを含まない行（空行やテキスト行）はテーブル範囲から除外
                    if (!text.includes('|')) {
...
module.exports = {
    tablePlugin: [tableField, tableStyles]
};